<template>
  <div :class="['yuan-tree-item-box']">
    <li>
       <!-- :style="{'padding-left':index*15+'px'}" -->
      <div :class="['yuan-tree-lable-box',{'label-active':tabIndexs === items.key,'yuan-tree-lable-disabled':items.disabled}]" @click.stop.prevent="toggle(items)" :data-key="dataKey" :style="{'padding-left':index*15+'px'}">
        <span :class="[{rotate:icon!='m-icon-arrow-right-filling'?false:items.isOpen},'yuan-tree-lable-span']"><i :class="[{'m-icon-arrow-right-filling':items.children&&items.children.length}]"></i></span>
        <span :class="['yuan-tree-label-select-span',{'yuan-tree-label-select-span-active':items.isSelected,'yuan-tree-label-select-span-active-1':items.semiSelected}]" @click.stop="selectClick(items,index)" v-if="multiple"></span>
        <span class="yuan-tree-label">{{items.label}}</span>
      </div>
      <transition name="slide-fade">
        <div class="yuan-tree-ul-box" v-show="items.isOpen" v-if="items.children&&items.children.length">
...
.yuan-tree-item-box{
  ...
  li{
    ...
    .yuan-tree-lable-box{
      ...
      .yuan-tree-label-select-span-active{
      ...
      .yuan-tree-label-select-span-active-1{
        width:14px;
        height: 14px;
        border-radius: 2px;
        display: inline-block;
