<template>
  <div class="yuan-progress">
    <div v-if="progressType === 'line'" class="yuan-progress-default">
      ...
      <div class="yuan-progress-status" v-show="!isShowText">
        <span>
          <slot>
            {{ formatPercent.text }}
          </slot>
    ...
    >
      ...
      <div v-if="isShowText" class="yuan-progress-circle-slot" :style="progressDashboardStyle">
        <slot>
          {{ progressCircleText }}
        </slot>
      </div>
...
const backClass = computed(() => {
  return ["yuan-progress-back", props.roundCorner ? "yuan-progress-b-r" : ""];
});
const speedClass = computed(() => {
  return props.progressType === "line"
    ? [
        "yuan-progress-speed",
        props.isAnimation ? "yuan-progress-animation" : "",
        props.roundCorner ? "yuan-progress-b-r" : "",
      ]
    : [
        "yuan-progress-circle",
        props.isAnimation ? "yuan-progress-circle-animation" : "",
        props.roundCorner ? "yuan-progress-circle-b-r" : "",
      ];
});
let girth = "";
...
.yuan-progress-default .yuan-progress-back {
  height: 8px;
  display: flex;
  align-items: center;
...
.yuan-progress-default .yuan-progress-back .yuan-progress-speed {
  position: absolute;
  left: 0;
  top: 0;
...
.yuan-progress-default .yuan-progress-back .yuan-progress-speed span {
  color: #fff;
  font-size: 12px;
  padding-right: 5px;
}
.yuan-progress-default .yuan-progress-status {
  min-width: 50px;
  padding-left: 8px;
  line-height: 1;
}
.yuan-progress-default-circle {
  display: inline-block;
  position: relative;
}
.yuan-progress-default-circle .yuan-progress-circle-slot {
  position: absolute;
  top: 50%;
  left: 50%;
...
.yuan-progress-circle-b-r {
  stroke-linecap: round;
}
.yuan-progress-animation {
  animation: infiniteLine 3s ease-out infinite;
}
.yuan-progress-circle-animation {
