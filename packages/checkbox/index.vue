<template>
  <div :class="[`yuan-checkbox-${size}`,customClass]">
    <label v-for="(item,index) in checkOptions" :key="index" class="yuan-checkbox-label" :class="[{'yuan-checkbox-label-active':item.checked},item.checked?item.disabled?'yuan-checkbox-label-active-disabled':'':item.disabled?'yuan-checkbox-label-disabled':'']" @click="checkChange(item,index)"  :style="{'display':inline?'inline-block':'table','margin-bottom':inline?'0':size=='small'?'8px':size=='mini'?'6px':'10px'}">
      <span class="yuan-checkbox-selectbox"  :style="{'border-color':customColor==''?'':item.disabled?'':item.checked?customColor:'','background':customColor==''?'':item.disabled?'':item.checked?customColor:''}"></span>
...
.yuan-checkbox-default{
  ...
  .yuan-checkbox-label-active-disabled{
    cursor: no-drop;
    span{
      color:#b4b6b9
    }
